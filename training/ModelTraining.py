...
structures = ["juxtaposition", "fronting", "inversion", "embedding"]
for structure in structures:
    happy_tc = HappyTextClassification(model_type="DISTILBERT",
                                   model_name="distilbert-base-uncased-finetuned-sst-2-english",
                                   num_labels=2)
    before_loss = happy_tc.eval(structure + "_dataset.csv").loss
    happy_tc.train(structure + "_dataset.csv", args=args)
    after_loss = happy_tc.eval(structure + "_dataset.csv").loss
